  ...
  for (;;) {
    ...
    if (consume("typedef")) {
      ...
      if (base_type || user_type) // 既に型を見つけた状態で、見てるトークンが型じゃない時
        break;
      Type *ty = find_typedef(token); // typedefで自分が定義した型
      if (!ty)
        break;
      token = token->next;
      user_type = ty;
...
//
//
// enum-list = enum-elm ("," enum-elm)* ","?
// enum-elm = ident ("=" const-expr)?
Type *enum_specifier() {
  expect("enum");
  Type *ty = enum_type();
...
// returns a node represents indices 3 and 4, this function
...
Node *stmt() {
  ...
  if (tok = consume("for")) {
    ...
    if (!consume(";")) {
      if (is_typename()) {
        node->init = declaration(); // for (int i=0; ....)
      } else {
        node->init = read_expr_stmt(); // for (i=0; ...)
        expect(";");
      }
    }
    if (!consume(";")) {
...
Node *conditional() {
  ...
  ternary->cond = node;
  ternary->then = expr(); // 条件がtrueの時の式
  expect(":");
  ternary->els = conditional(); // 条件がfalseの時の式
  return ternary;
}
